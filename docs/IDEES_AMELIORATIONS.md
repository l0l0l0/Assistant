...
> **Statut 2026-06-12 (même jour)** : items **1.1, 1.2, 1.3 implémentés** (+ bonus : `Config::save()` à l'arrêt — les réglages ControlPanel n'étaient jamais persistés), puis **3.1 (focus assist), 3.2 (fichiers récents + auto-reload), 2.1 (RemoteView câblé)**, l'essentiel de **3.4** (onglet Settings « Features » ; manquent detector model dropdown / dark mode / checkbox columns), puis **3.3 (restauration session d'inspection)** et **2.2 (boutons Report HTML/PDF)**. Détail dans [JETSON_SESSION_LOG.md](JETSON_SESSION_LOG.md). ⚠️ Non compilé ici (pas de toolchain) — à valider sur le Jetson : `bash scripts/build_jetson.sh` + `ctest`.
