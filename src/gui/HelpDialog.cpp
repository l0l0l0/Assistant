...
void HelpDialog::createTabs()
{
    ...
    addPage(tr("Shortcuts"), tr(
        ...
        "<tr><td><b>P</b></td><td>Guided tour: mark current component placed &amp; advance</td></tr>"
        "<tr><td><b>N</b> / <b>Shift+N</b></td><td>Guided tour: next (skip) / previous component</td></tr>"
        "<tr><td><b>Ctrl+Z</b></td><td>Undo the last placement of this session</td></tr>"
    ...
    addPage(tr("Inspection"), tr(
        ...
        "<h3>Guided tour (hands-free)</h3>"
        "<p><i>Inspection → Start Inspection</i> (I) walks through components one by one; the "
        "current target is highlighted in the overlay, the BOM and the PCB Map (which recenters "
        "on it when zoomed). With <i>Settings → Component order → Nearest neighbor</i> the route "
        "minimizes travel across the board. Keep your hands on the board and drive with the "
        "keyboard: <b>P</b> = placed &amp; next, <b>N</b> = skip, <b>Shift+N</b> = back, "
        "<b>Ctrl+Z</b> = undo the last placement. Every action is journaled to "
        "<code>inspection_log.csv</code> (timestamp, board, face, action, ref) and progress is "
        "persisted — a restart resumes exactly where you stopped.</p>"

        "<h3>Revision diff (update rework)</h3>"
        "<p><i>Inspection → Compare with Another Revision…</i> diffs the loaded iBOM against "
        "another revision's file: components to <b>remove</b> (desolder), to <b>add</b>, and to "
        "<b>change</b> (value, footprint, side, or position beyond 0.5 mm) — the work list for "
        "bringing a rev-A board to rev B.</p>"
